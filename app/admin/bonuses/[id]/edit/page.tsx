...
export default function EditBonusPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/bonuses', { credentials: 'include' })
      .then((res) => res.json())
      .then((data) => {
        const bonus = data.bonuses.find((b: any) => b.id === id);
        if (bonus) {
          setFormData({
            title: bonus.title || '',
            slug: bonus.slug || '',
            ...
            casinoRating: bonus.casinoRating || 4.5,
            amount: bonus.amount || '',
            type: bonus.type || 'welcome',
            wagering: bonus.wagering || '',
            minDeposit: bonus.minDeposit || '',
            code: bonus.code || '',
            expiry: bonus.expiry ? new Date(bonus.expiry).toISOString().split('T')[0] : '',
            isExclusive: bonus.isExclusive || false,
            isHotPick: bonus.isHotPick || false,
            isActive: bonus.isActive ?? true,
            provider: bonus.provider || '',
            features: bonus.features || [],
          });
          if (bonus.casinoId) {
            setSelectedCasinoId(bonus.casinoId);
          }
        }
        setLoading(false);
      })
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-white">Edit Bonus</h1>
      </div>
